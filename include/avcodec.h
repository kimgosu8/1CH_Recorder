 ...
 * The fields of the given AVPicture are filled in by using the 'ptr' address which points to the image data buffer. 
 * Depending on the specified picture format, one or multiple image data pointers and line sizes will be set.
 * If a planar format is specified, several pointers will be set pointing to the different picture planes 
 * and the line sizes of the different planes will be stored in the lines_sizes array.
 *
 ...
 * The avcodec_encode_video() function encodes a video frame from the input \p pict.
 * To encode it, it makes use of the video codec which was coupled with \p avctx using avcodec_open(). 
 * The resulting encoded bytes representing the frame are stored in the output buffer \p buf. 
 * The input picture should be stored using a specific format, namely \c avctx.pix_fmt.
 *
 * @param avctx the codec context
